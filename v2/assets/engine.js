/* Motor dos Jogos da Bibel — v2. Le window.__G__ (dados do jogo, iguais aos da v1) e desenha home + jogo.
   Diferencas da v1: emoji do conteudo renderizado como SVG via glyph.js, entrada animada das opcoes,
   pop no acerto, cascata no tabuleiro da memoria. Nenhuma mudanca no formato do dado.
   Idioma BR/EN via chave localStorage "bibel_lang", compartilhada com a v1. */
(function(){
var UI={
 ...
 function show(){
  ...
  stageEl.querySelectorAll('.opt').forEach(function(btn){
   btn.onclick=function(){
    ...
    if(o.correct){
     btn.classList.add('ok','done','pop');var rc=btn.getBoundingClientRect();burst(rc.left+rc.width/2,rc.top+rc.height/2);
